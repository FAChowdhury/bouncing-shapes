...
#		pragma clang diagnostic ignored "-Wunsafe-buffer-usage" // warning: 'xxx' is an unsafe pointer used for buffer
		                                                         // access
...
typedef void (*ImGuiSizeCallback)(ImGuiSizeCallbackData* data); // Callback function for
...
namespace ImGui
{
	...
	IMGUI_API void ShowDemoWindow(bool* p_open = NULL); // create Demo window. demonstrate most ImGui features. call
	                                                    // this to learn about the library! try to make it always
	                                                    // available in your application!
	IMGUI_API void ShowMetricsWindow(bool* p_open = NULL); // create Metrics/Debugger window. display Dear ImGui
	                                                       // internals: windows, draw commands, various internal state,
	                                                       // etc.
	...
	IMGUI_API void ShowAboutWindow(bool* p_open = NULL); // create About window. display Dear ImGui version, credits and
	...
	IMGUI_API bool ShowStyleSelector(const char* label); // add style selector block (not a window), essentially a combo
	                                                     // listing the default styles.
	IMGUI_API void ShowFontSelector(const char* label); // add font selector block (not a window), essentially a combo
	...
	IMGUI_API bool IsWindowHovered(ImGuiHoveredFlags flags = 0); // is current window hovered and hoverable (e.g. not
	                                                             // blocked by a popup/modal)? See ImGuiHoveredFlags_
	                                                             // for options. IMPORTANT: If you are trying to check
	                                                             // whether your mouse should be dispatched to Dear
	                                                             // ImGui or to your underlying app, you should not use
	                                                             // this function! Use the 'io.WantCaptureMouse' boolean
	                                                             // for that! Refer to FAQ entry "How can I tell whether
	                                                             // to dispatch mouse/keyboard to Dear ImGui or my
	                                                             // application?" for details.
	IMGUI_API ImDrawList* GetWindowDrawList(); // get draw list associated to the current window, to append your own
	                                           // drawing primitives
	IMGUI_API ImVec2 GetWindowPos(); // get current window position in screen space (IT IS UNLIKELY YOU EVER NEED TO USE
	                                 // THIS. Consider always using GetCursorScreenPos() and GetContentRegionAvail()
	                                 // instead)
	IMGUI_API ImVec2 GetWindowSize(); // get current window size (IT IS UNLIKELY YOU EVER NEED TO USE THIS. Consider
	                                  // always using GetCursorScreenPos() and GetContentRegionAvail() instead)
	IMGUI_API float GetWindowWidth(); // get current window width (IT IS UNLIKELY YOU EVER NEED TO USE THIS). Shortcut
	                                  // for GetWindowSize().x.
	IMGUI_API float GetWindowHeight(); // get current window height (IT IS UNLIKELY YOU EVER NEED TO USE THIS). Shortcut
	                                   // for GetWindowSize().y.

	// Window manipulation
	// - Prefer using SetNextXXX functions (before Begin) rather that SetXXX functions (after Begin).
	IMGUI_API void
	SetNextWindowPos(const ImVec2& pos, ImGuiCond cond = 0, const ImVec2& pivot = ImVec2(0, 0)); // set next window
	                                                                                             // position. call
	                                                                                             // before Begin(). use
	                                                                                             // pivot=(0.5f,0.5f) to
	                                                                                             // center on given
	                                                                                             // point, etc.
	IMGUI_API void SetNextWindowSize(const ImVec2& size, ImGuiCond cond = 0); // set next window size. set axis to 0.0f
	                                                                          // to force an auto-fit on this axis. call
	                                                                          // before Begin()
	IMGUI_API void SetNextWindowSizeConstraints(const ImVec2& size_min,
	                                            ...
	                                            void* custom_callback_data = NULL); // set next window size limits. use
	                                                                                // 0.0f or FLT_MAX if you don't want
	                                                                                // limits. Use -1 for both min and
	                                                                                // max of same axis to preserve
	                                                                                // current size (which itself is a
	                                                                                // constraint). Use callback to
	                                                                                // apply non-trivial programmatic
	                                                                                // constraints.
	IMGUI_API void SetNextWindowContentSize(const ImVec2& size); // set next window content size (~ scrollable client
	...
	IMGUI_API void SetNextWindowScroll(const ImVec2& scroll); // set next window scrolling value (use < 0.0f to not
	                                                          // affect a given axis).
	IMGUI_API void SetNextWindowBgAlpha(float alpha); // set next window background color alpha. helper to easily
	                                                  // override the Alpha component of
	                                                  // ImGuiCol_WindowBg/ChildBg/PopupBg. you may also use
	                                                  // ImGuiWindowFlags_NoBackground.
	IMGUI_API void SetWindowPos(const ImVec2& pos, ImGuiCond cond = 0); // (not recommended) set current window position
	...
	IMGUI_API void SetWindowSize(const ImVec2& size, ImGuiCond cond = 0); // (not recommended) set current window size -
	                                                                      // call within Begin()/End(). set to ImVec2(0,
	                                                                      // 0) to force an auto-fit. prefer using
	                                                                      // SetNextWindowSize(), as this may incur
	...
	IMGUI_API void SetWindowFocus(); // (not recommended) set current window to be focused / top-most. prefer using
	                                 // SetNextWindowFocus().
	IMGUI_API void SetWindowFontScale(float scale); // [OBSOLETE] set font scale. Adjust IO.FontGlobalScale if you want
	...
	IMGUI_API void SetWindowSize(const char* name, const ImVec2& size, ImGuiCond cond = 0); // set named window size.
	                                                                                        // set axis to 0.0f to force
	                                                                                        // an auto-fit on this axis.
	IMGUI_API void SetWindowCollapsed(const char* name, bool collapsed, ImGuiCond cond = 0); // set named window
	                                                                                         // collapsed state
	IMGUI_API void SetWindowFocus(const char* name); // set named window to be focused / top-most. use NULL to remove
	                                                 // focus.
	...
	IMGUI_API void SetScrollHereX(float center_x_ratio = 0.5f); // adjust scrolling amount to make current cursor
	                                                            // position visible. center_x_ratio=0.0: left, 0.5:
	                                                            // center, 1.0: right. When using to make a
	                                                            // "default/current item" visible, consider using
	...
	IMGUI_API void SetScrollHereY(float center_y_ratio = 0.5f); // adjust scrolling amount to make current cursor
	                                                            // position visible. center_y_ratio=0.0: top, 0.5:
	                                                            // center, 1.0: bottom. When using to make a
	                                                            // "default/current item" visible, consider using
	                                                            // SetItemDefaultFocus() instead.
	IMGUI_API void SetScrollFromPosX(float local_x, float center_x_ratio = 0.5f); // adjust scrolling amount to make
	                                                                              // given position visible. Generally
	                                                                              // GetCursorStartPos() + offset to
	                                                                              // compute a valid position.
	IMGUI_API void SetScrollFromPosY(float local_y, float center_y_ratio = 0.5f); // adjust scrolling amount to make
	                                                                              // given position visible. Generally
	                                                                              // GetCursorStartPos() + offset to
	                                                                              // compute a valid position.
	...
	IMGUI_API void PushStyleColor(ImGuiCol idx, ImU32 col); // modify a style color. always use this if you modify the
	...
	IMGUI_API void PushStyleVar(ImGuiStyleVar idx, const ImVec2& val); // modify a style ImVec2 variable. always use
	                                                                   // this if you modify the style after NewFrame().
	IMGUI_API void PopStyleVar(int count = 1);
	IMGUI_API void PushItemFlag(ImGuiItemFlags option, bool enabled); // modify specified shared item flag, e.g.
	...
	IMGUI_API void PushTextWrapPos(float wrap_local_pos_x = 0.0f); // push word-wrapping position for Text*() commands.
	                                                               // < 0.0f: no wrapping; 0.0f: wrap to end of window
	                                                               // (or column); > 0.0f: wrap at 'wrap_pos_x' position
	...
	IMGUI_API ImU32 GetColorU32(ImGuiCol idx, float alpha_mul = 1.0f); // retrieve given style color with style alpha
	                                                                   ...
	                                                                   // packed as a 32-bit value suitable for
	                                                                   // ImDrawList
	...
	IMGUI_API const ImVec4& GetStyleColorVec4(ImGuiCol idx); // retrieve style color as stored in ImGuiStyle structure.
	...
	IMGUI_API ImVec2 GetCursorScreenPos(); // cursor position, absolute coordinates. THIS IS YOUR BEST FRIEND (prefer
	                                       // using this rather than GetCursorPos(), also more useful to work with
	                                       // ImDrawList API).
	IMGUI_API void SetCursorScreenPos(const ImVec2& pos); // cursor position, absolute coordinates. THIS IS YOUR BEST
	                                                      // FRIEND.
	IMGUI_API ImVec2 GetContentRegionAvail(); // available space from current position. THIS IS YOUR BEST FRIEND.
	IMGUI_API ImVec2 GetCursorPos(); // [window-local] cursor position in window-local coordinates. This is not your
	                                 // best friend.
	...
	IMGUI_API void SameLine(float offset_from_start_x = 0.0f, float spacing = -1.0f); // call between widgets or groups
	                                                                                  // to layout them horizontally. X
	                                                                                  // position given in window
	                                                                                  // coordinates.
	...
	IMGUI_API void PushID(const char* str_id_begin, const char* str_id_end); // push string into the ID stack (will hash
	                                                                         // string).
	...
	// Widgets: Text
	IMGUI_API void TextUnformatted(const char* text, const char* text_end = NULL); // raw text without formatting.
	                                                                               // Roughly equivalent to Text("%s",
	                                                                               // text) but: A) doesn't require null
	                                                                               // terminated string if 'text_end' is
	                                                                               // specified, B) it's faster, no
	                                                                               // memory copy is done, no buffer
	                                                                               // size limits, recommended for long
	                                                                               // chunks of text.
	...
	IMGUI_API void TextDisabled(const char* fmt, ...) IM_FMTARGS(1); // shortcut for PushStyleColor(ImGuiCol_Text,
	                                                                 // style.Colors[ImGuiCol_TextDisabled]); Text(fmt,
	                                                                 // ...); PopStyleColor();
	IMGUI_API void TextDisabledV(const char* fmt, va_list args) IM_FMTLIST(1);
	IMGUI_API void TextWrapped(const char* fmt, ...) IM_FMTARGS(1); // shortcut for PushTextWrapPos(0.0f); Text(fmt,
	                                                                // ...); PopTextWrapPos();. Note that this won't
	                                                                // work on an auto-resizing window if there's no
	                                                                // other widgets to extend the window width, yoy may
	                                                                // need to set a size using SetNextWindowSize().
	IMGUI_API void TextWrappedV(const char* fmt, va_list args) IM_FMTLIST(1);
	IMGUI_API void LabelText(const char* label, const char* fmt, ...) IM_FMTARGS(2); // display text+label aligned the
	                                                                                 // same way as value+label widgets
	...
	IMGUI_API bool
	InvisibleButton(const char* str_id, const ImVec2& size, ImGuiButtonFlags flags = 0); // flexible button behavior
	                                                                                     // without the visuals,
	                                                                                     // frequently useful to build
	                                                                                     // custom behaviors using the
	...
	IMGUI_API bool RadioButton(const char* label, bool active); // use with e.g. if (RadioButton("one", my_value==1)) {
	                                                            // my_value = 1; }
	IMGUI_API bool RadioButton(const char* label, int* v, int v_button); // shortcut to handle the above pattern when
	                                                                     // value is an integer
	...
	IMGUI_API void TextLinkOpenURL(const char* label, const char* url = NULL); // hyperlink text button, automatically
	...
	IMGUI_API bool ColorButton(const char* desc_id,
	                           ...
	                           const ImVec2& size = ImVec2(0, 0)); // display a color square/button, hover for details,
	...
	IMGUI_API void SetColorEditOptions(ImGuiColorEditFlags flags); // initialize current options (generally on
	                                                               ...
	                                                               // able to change many settings, unless you pass the
	...
	IMGUI_API bool TreeNode(const char* str_id, const char* fmt, ...)
	    IM_FMTARGS(2); // helper variation to easily decorelate the id from the displayed string. Read the FAQ about why
	                   // and how to use ID. to align arbitrary text at the same level as a TreeNode() you can use
	                   // Bullet().
	...
	IMGUI_API float GetTreeNodeToLabelSpacing(); // horizontal distance preceding label when using TreeNode*() or
	                                             ...
	                                             // unframed TreeNode
	IMGUI_API bool CollapsingHeader(const char* label, ImGuiTreeNodeFlags flags = 0); // if returning 'true' the header
	                                                                                  // is open. doesn't indent nor
	                                                                                  // push on ID stack. user doesn't
	                                                                                  // have to call TreePop().
	IMGUI_API bool
	CollapsingHeader(const char* label, bool* p_visible, ImGuiTreeNodeFlags flags = 0); // when 'p_visible != NULL': if
	                                                                                    // '*p_visible==true' display an
	                                                                                    // additional small close button
	                                                                                    // on upper right of the header
	                                                                                    // which will set the bool to
	                                                                                    // false when clicked, if
	                                                                                    // '*p_visible==false' don't
	                                                                                    // display the header.
	...
	IMGUI_API bool Selectable(const char* label,
	                          ...
	                          const ImVec2& size = ImVec2(0, 0)); // "bool selected" carry the selection state
	                                                              // (read-only). Selectable() is clicked is returns
	                                                              // true so you can modify your selection state.
	                                                              // size.x==0.0: use remaining width, size.x>0.0:
	                                                              // specify width. size.y==0.0: use label height,
	...
	// - (*) TreeNode() is technically supported but... using this correctly is more complicate: you need some sort of
	// linear/random access to your tree,
	//   which is suited to advanced trees setups already implementing filters and clipper. We will work toward
	...
	IMGUI_API bool IsItemToggledSelection(); // Was the last item selection state toggled? Useful if you need the
	                                         // per-item information _before_ reaching EndMultiSelect(). We only returns
	                                         // toggle _event_ in order to handle clipping correctly.
	...
	IMGUI_API bool
	MenuItem(const char* label, const char* shortcut = NULL, bool selected = false, bool enabled = true); // return true
	                                                                                                      // when
	                                                                                                      // activated.
	IMGUI_API bool
	MenuItem(const char* label, const char* shortcut, bool* p_selected, bool enabled = true); // return true when
	                                                                                          // activated + toggle
	                                                                                          // (*p_selected) if
	                                                                                          // p_selected != NULL
	...
	IMGUI_API void SetTooltip(const char* fmt, ...) IM_FMTARGS(1); // set a text-only tooltip. Often used after a
	                                                               // ImGui::IsItemHovered() check. Override any
	...
	IMGUI_API void SetItemTooltip(const char* fmt, ...) IM_FMTARGS(1); // set a text-only tooltip if preceding item was
	                                                                   // hovered. override any previous call to
	                                                                   // SetTooltip().
	...
	IMGUI_API bool BeginPopup(const char* str_id, ImGuiWindowFlags flags = 0); // return true if the popup is open, and
	                                                                           // you can start outputting to it.
	IMGUI_API bool BeginPopupModal(const char* name, bool* p_open = NULL, ImGuiWindowFlags flags = 0); // return true if
	                                                                                                   // the modal is
	                                                                                                   // open, and you
	                                                                                                   // can start
	                                                                                                   // outputting to
	...
	IMGUI_API void OpenPopup(const char* str_id, ImGuiPopupFlags popup_flags = 0); // call to mark popup as open (don't
	                                                                               // call every frame!).
	IMGUI_API void OpenPopup(ImGuiID id, ImGuiPopupFlags popup_flags = 0); // id overload to facilitate calling from
	                                                                       // nested stacks
	IMGUI_API void OpenPopupOnItemClick(const char* str_id = NULL,
	                                    ImGuiPopupFlags popup_flags = 1); // helper to open popup when clicked on last
	                                                                      // item. Default to
	                                                                      // ImGuiPopupFlags_MouseButtonRight == 1.
	                                                                      // (note: actually triggers on the mouse
	                                                                      // _released_ event to be consistent with
	                                                                      // popup behaviors)
	...
	IMGUI_API bool BeginPopupContextItem(const char* str_id = NULL,
	                                     ImGuiPopupFlags popup_flags = 1); // open+begin popup when clicked on last
	                                                                       // item. Use str_id==NULL to associate the
	                                                                       // popup to previous item. If you want to use
	                                                                       // that on a non-interactive item such as
	                                                                       // Text() you need to pass in an explicit ID
	                                                                       // here. read comments in .cpp!
	IMGUI_API bool BeginPopupContextWindow(const char* str_id = NULL, ImGuiPopupFlags popup_flags = 1); // open+begin
	                                                                                                    // popup when
	                                                                                                    // clicked on
	                                                                                                    // current
	                                                                                                    // window.
	IMGUI_API bool BeginPopupContextVoid(const char* str_id = NULL, ImGuiPopupFlags popup_flags = 1); // open+begin
	                                                                                                  // popup when
	                                                                                                  // clicked in void
	                                                                                                  // (where there
	                                                                                                  // are no
	...
	IMGUI_API void TableNextRow(ImGuiTableRowFlags row_flags = 0, float min_row_height = 0.0f); // append into the first
	                                                                                            // cell of a new row.
	...
	IMGUI_API ImGuiTableColumnFlags TableGetColumnFlags(int column_n = -1); // return column flags so you can query
	                                                                        // their Enabled/Visible/Sorted/Hovered
	                                                                        // status flags. Pass -1 to use current
	                                                                        // column.
	IMGUI_API void TableSetColumnEnabled(int column_n, bool v); // change user accessible enabled/disabled state of a
	                                                            // column. Set to false to hide the column. User can use
	                                                            // the context menu to change this themselves
	                                                            // (right-click in headers, or right-click in columns
	...
	IMGUI_API void TableSetBgColor(ImGuiTableBgTarget target, ImU32 color, int column_n = -1); // change the color of a
	                                                                                           // cell, row, or column.
	                                                                                           // See
	                                                                                           // ImGuiTableBgTarget_
	                                                                                           // flags for details.
	...
	IMGUI_API float GetColumnWidth(int column_index = -1); // get column width (in pixels). pass -1 to use current
	                                                       // column
	IMGUI_API void SetColumnWidth(int column_index, float width); // set column width (in pixels). pass -1 to use
	                                                              // current column
	IMGUI_API float GetColumnOffset(int column_index = -1); // get position of column line (in pixels, from the left
	                                                        // side of the contents region). pass -1 to use current
	                                                        // column, otherwise 0..GetColumnsCount() inclusive. column
	                                                        // 0 is typically 0.0f
	IMGUI_API void SetColumnOffset(int column_index, float offset_x); // set position of column line (in pixels, from
	                                                                  // the left side of the contents region). pass -1
	...
	IMGUI_API bool BeginTabItem(const char* label, bool* p_open = NULL, ImGuiTabItemFlags flags = 0); // create a Tab.
	                                                                                                  // Returns true if
	                                                                                                  // the Tab is
	                                                                                                  // selected.
	IMGUI_API void EndTabItem(); // only call EndTabItem() if BeginTabItem() returns true!
	IMGUI_API bool TabItemButton(const char* label, ImGuiTabItemFlags flags = 0); // create a Tab behaving like a
	                                                                              // button. return true when clicked.
	                                                                              // cannot be selected in the tab bar.
	IMGUI_API void SetTabItemClosed(const char* tab_or_docked_window_label); // notify TabBar or Docking system of a
	...
	IMGUI_API bool BeginDragDropSource(ImGuiDragDropFlags flags = 0); // call after submitting an item which may be
	                                                                  // dragged. when this return true, you can call
	                                                                  // SetDragDropPayload() + EndDragDropSource()
	IMGUI_API bool
	SetDragDropPayload(const char* type, const void* data, size_t sz, ImGuiCond cond = 0); // type is a user defined
	                                                                                       // string of maximum 32
	                                                                                       // characters. Strings
	                                                                                       // starting with '_' are
	                                                                                       // reserved for dear imgui
	                                                                                       // internal types. Data is
	                                                                                       // copied and held by imgui.
	                                                                                       // Return true when payload
	                                                                                       // has been accepted.
	...
	AcceptDragDropPayload(const char* type, ImGuiDragDropFlags flags = 0); // accept contents of a given type. If
	                                                                       // ImGuiDragDropFlags_AcceptBeforeDelivery is
	                                                                       // set you can peek into the payload before
	...
	IMGUI_API void SetKeyboardFocusHere(int offset = 0); // focus keyboard on the next widget. Use positive 'offset' to
	                                                     // access sub components of a multiple component widget. Use -1
	                                                     // to access previous widget.
	...
	IMGUI_API bool IsItemHovered(ImGuiHoveredFlags flags = 0); // is the last item hovered? (and usable, aka not blocked
	                                                           // by a popup, etc.). See ImGuiHoveredFlags for more
	                                                           // options.
	...
	IMGUI_API bool IsRectVisible(const ImVec2& rect_min, const ImVec2& rect_max); // test if rectangle (in screen space)
	                                                                              // is visible / not clipped. to
	                                                                              // perform coarse clipping on user's
	                                                                              // side.
	...
	IMGUI_API ImDrawListSharedData* GetDrawListSharedData(); // you may use this when creating your own ImDrawList
	                                                         // instances.
	IMGUI_API const char* GetStyleColorName(ImGuiCol idx); // get a string corresponding to the enum value (for display,
	                                                       // saving, etc.).
	IMGUI_API void SetStateStorage(ImGuiStorage* storage); // replace current window storage with our own (if you want
	                                                       // to manipulate it yourself, typically clear subsection of
	                                                       // it)
	...
	IMGUI_API bool IsKeyChordPressed(ImGuiKeyChord key_chord); // was key chord (mods + key) pressed, e.g. you can pass
	...
	IMGUI_API int GetKeyPressedAmount(ImGuiKey key, float repeat_delay, float rate); // uses provided repeat rate/delay.
	                                                                                 // return a count, most often 0 or
	                                                                                 // 1 but might be >1 if RepeatRate
	                                                                                 // is small enough that DeltaTime >
	                                                                                 // RepeatRate
	IMGUI_API const char* GetKeyName(ImGuiKey key); // [DEBUG] returns English name of the key. Those names a provided
	                                                // for debugging purpose and are not meant to be saved persistently
	                                                // not compared.
	IMGUI_API void SetNextFrameWantCaptureKeyboard(bool want_capture_keyboard); // Override io.WantCaptureKeyboard flag
	...
	IMGUI_API void SetItemKeyOwner(ImGuiKey key); // Set key owner to last item ID if it is hovered or active.
	...
	IMGUI_API bool IsMouseClicked(ImGuiMouseButton button, bool repeat = false); // did mouse button clicked? (went from
	                                                                             // !Down to Down). Same as
	...
	IMGUI_API bool IsMouseDoubleClicked(ImGuiMouseButton button); // did mouse button double-clicked? Same as
	                                                              // GetMouseClickedCount() == 2. (note that a
	                                                              // double-click will also report IsMouseClicked() ==
	                                                              // true)
	...
	IMGUI_API bool
	IsMouseHoveringRect(const ImVec2& r_min, const ImVec2& r_max, bool clip = true); // is mouse hovering given bounding
	                                                                                 // rect (in screen space). clipped
	                                                                                 // by current clipping settings,
	                                                                                 // but disregarding of other
	                                                                                 // consideration of focus/window
	...
	IMGUI_API ImVec2 GetMousePos(); // shortcut to ImGui::GetIO().MousePos provided by user, to be consistent with other
	                                // calls
	IMGUI_API ImVec2 GetMousePosOnOpeningCurrentPopup(); // retrieve mouse position at the time of opening popup we have
	                                                     ...
	                                                     // themselves)
	IMGUI_API bool IsMouseDragging(ImGuiMouseButton button, float lock_threshold = -1.0f); // is mouse dragging? (uses
	...
	IMGUI_API ImVec2 GetMouseDragDelta(ImGuiMouseButton button = 0,
	                                   float lock_threshold = -1.0f); // return the delta from the initial clicking
	                                                                  ...
	                                                                  // was just released. This is locked and return
	                                                                  // 0.0f until the mouse moves past a distance
	                                                                  // threshold at least once (uses
	                                                                  // io.MouseDraggingThreshold if lock_threshold <
	                                                                  // 0.0f)
	IMGUI_API void ResetMouseDragDelta(ImGuiMouseButton button = 0); //
	IMGUI_API ImGuiMouseCursor GetMouseCursor(); // get desired mouse cursor shape. Important: reset in
	...
	IMGUI_API void SetNextFrameWantCaptureMouse(bool want_capture_mouse); // Override io.WantCaptureMouse flag next
	...
	IMGUI_API void LoadIniSettingsFromMemory(const char* ini_data, size_t ini_size = 0); // call after CreateContext()
	                                                                                     // and before the first call to
	                                                                                     // NewFrame() to provide .ini
	                                                                                     // data from your own data
	                                                                                     // source.
	...
	IMGUI_API const char* SaveIniSettingsToMemory(size_t* out_ini_size = NULL); // return a zero-terminated string with
	                                                                            // the .ini data which you can save by
	                                                                            // your own mean. call when
	                                                                            // io.WantSaveIniSettings is set, then
	                                                                            // save data by your own mean and clear
...
#	ifndef IMGUI_DISABLE_DEBUG_TOOLS
	IMGUI_API void DebugLog(const char* fmt, ...) IM_FMTARGS(1); // Call via IMGUI_DEBUG_LOG() for maximum stripping in
	                                                             // caller code!
...
enum ImGuiWindowFlags_
{
	...
	ImGuiWindowFlags_NoScrollbar = 1 << 3, // Disable scrollbars (window can still scroll with mouse or
	                                       // programmatically)
	ImGuiWindowFlags_NoScrollWithMouse = 1 << 4, // Disable user vertically scrolling with mouse wheel. On child window,
	                                             // mouse wheel will be forwarded to the parent unless NoScrollbar is
	                                             // also set.
	...
	ImGuiWindowFlags_NoFocusOnAppearing = 1 << 12, // Disable taking focus when transitioning from hidden to visible
	                                               // state
	...
	ImGuiWindowFlags_AlwaysVerticalScrollbar = 1 << 14, // Always show vertical scrollbar (even if ContentSize.y <
	                                                    // Size.y)
	...
	ImGuiWindowFlags_NoNavFocus = 1 << 17, // No focusing toward this window with gamepad/keyboard navigation (e.g.
	                                       // skipped by CTRL+TAB)
	ImGuiWindowFlags_UnsavedDocument = 1 << 18, // Display a dot next to the title. When used in a tab/docking context,
...
#	ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
	ImGuiWindowFlags_AlwaysUseWindowPadding = 1 << 30, // Obsoleted in 1.90.0: Use
	                                                   // ImGuiChildFlags_AlwaysUseWindowPadding in BeginChild() call.
	ImGuiWindowFlags_NavFlattened = 1 << 31, // Obsoleted in 1.90.9: Use ImGuiChildFlags_NavFlattened in BeginChild()
	                                         // call.
#	endif
};
...
enum ImGuiChildFlags_
{
	...
	ImGuiChildFlags_AlwaysUseWindowPadding = 1 << 1, // Pad with style.WindowPadding even if no border are drawn (no
	...
	ImGuiChildFlags_AutoResizeX = 1 << 4, // Enable auto-resizing width. Read "IMPORTANT: Size measurement" details
	                                      // above.
	ImGuiChildFlags_AutoResizeY = 1 << 5, // Enable auto-resizing height. Read "IMPORTANT: Size measurement" details
	                                      // above.
	ImGuiChildFlags_AlwaysAutoResize = 1 << 6, // Combined with AutoResizeX/AutoResizeY. Always measure size even when
	                                           // child is hidden, always return true, always disable clipping
	                                           // optimization! NOT RECOMMENDED.
	ImGuiChildFlags_FrameStyle = 1 << 7, // Style the child window like a framed item: use FrameBg, FrameRounding,
...
enum ImGuiItemFlags_
{
	...
	ImGuiItemFlags_NoTabStop = 1 << 0, // false    // Disable keyboard tabbing. This is a "lighter" version of
	...
	ImGuiItemFlags_NoNavDefaultFocus = 1 << 2, // false    // Disable item being a candidate for default focus (e.g.
	...
	ImGuiItemFlags_AutoClosePopups = 1 << 4, // true     // MenuItem()/Selectable() automatically close their parent
	                                         // popup window.
};
...
enum ImGuiInputTextFlags_
{
	...
	ImGuiInputTextFlags_EnterReturnsTrue = 1 << 6, // Return 'true' when Enter is pressed (as opposed to every time the
	...
	ImGuiInputTextFlags_CtrlEnterForNewLine = 1 << 8, // In multi-line mode, validate with Enter, add new line with
	...
	ImGuiInputTextFlags_ParseEmptyRefVal = 1 << 13, // InputFloat(), InputInt(), InputScalar() etc. only: parse empty
	                                                // string as zero value.
	ImGuiInputTextFlags_DisplayEmptyRefVal = 1 << 14, // InputFloat(), InputInt(), InputScalar() etc. only: when value
	...
	ImGuiInputTextFlags_NoUndoRedo = 1 << 16, // Disable undo/redo. Note that input text owns the text data while
	...
	ImGuiInputTextFlags_CallbackAlways = 1 << 19, // Callback on each iteration. User code may query cursor position,
	                                              // modify text buffer.
	ImGuiInputTextFlags_CallbackCharFilter = 1 << 20, // Callback on character inputs to replace or discard them. Modify
	                                                  ...
	                                                  // discard.
	ImGuiInputTextFlags_CallbackResize = 1 << 21, // Callback on buffer capacity changes request (beyond 'buf_size'
	                                              // parameter value), allowing the string to grow. Notify when the
	                                              // string wants to be resized (for string types which hold a cache of
	                                              // their Size). You will be provided a new BufSize in the callback and
	                                              // NEED to honor it. (see misc/cpp/imgui_stdlib.h for an example of
	                                              // using this)
	ImGuiInputTextFlags_CallbackEdit = 1 << 22, // Callback on any edit (note that InputText() already returns true on
...
enum ImGuiTreeNodeFlags_
{
	...
	ImGuiTreeNodeFlags_SpanAvailWidth = 1 << 11, // Extend hit box to the right-most edge, even if not framed. This is
	                                             // not the default in order to allow adding other items on the same
	                                             // line without using AllowOverlap mode.
	ImGuiTreeNodeFlags_SpanFullWidth = 1 << 12, // Extend hit box to the left-most and right-most edges (cover the
	                                            // indent area).
	ImGuiTreeNodeFlags_SpanTextWidth = 1 << 13, // Narrow hit box + narrow hovering highlight, will only cover the label
	                                            // text.
	ImGuiTreeNodeFlags_SpanAllColumns = 1 << 14, // Frame will span all columns of its container table (text will still
...
enum ImGuiPopupFlags_
{
	...
	ImGuiPopupFlags_AnyPopupLevel = 1 << 11, // For IsPopupOpen(): search/test at any level of the popup stack (default
...
enum ImGuiSelectableFlags_
{
	...
	ImGuiSelectableFlags_NoAutoClosePopups = 1 << 0, // Clicking this doesn't close parent popup window (overrides
	                                                 // ImGuiItemFlags_AutoClosePopups)
	ImGuiSelectableFlags_SpanAllColumns = 1 << 1, // Frame will span all columns of its container table (text will still
	                                              // fit in current column)
...
enum ImGuiComboFlags_
{
	...
	ImGuiComboFlags_HeightSmall = 1 << 1, // Max ~4 items visible. Tip: If you want your combo popup to be a specific
...
enum ImGuiTabBarFlags_
{
	...
	ImGuiTabBarFlags_Reorderable = 1 << 0, // Allow manually dragging tabs to re-order them + New tabs are appended at
	                                       // the end of list
...
enum ImGuiTabItemFlags_
{
	...
	ImGuiTabItemFlags_SetSelected = 1 << 1, // Trigger flag to programmatically make the tab selected when calling
	                                        // BeginTabItem()
	ImGuiTabItemFlags_NoCloseWithMiddleMouseButton = 1 << 2, // Disable behavior of closing tabs (that are submitted
	...
	ImGuiTabItemFlags_Trailing = 1 << 7, // Enforce the tab position to the right of the tab bar (before the scrolling
	                                     // buttons)
	ImGuiTabItemFlags_NoAssumedClosure = 1 << 8, // Tab is selected when trying to close + closure is not immediately
...
enum ImGuiHoveredFlags_
{
	...
	ImGuiHoveredFlags_ChildWindows = 1 << 0, // IsWindowHovered() only: Return true if any children of the window is
	                                         // hovered
	ImGuiHoveredFlags_RootWindow = 1 << 1, // IsWindowHovered() only: Test from root window (top most parent of the
	                                       // current hierarchy)
	ImGuiHoveredFlags_AnyWindow = 1 << 2, // IsWindowHovered() only: Return true if any window is hovered
	ImGuiHoveredFlags_NoPopupHierarchy = 1 << 3, // IsWindowHovered() only: Do not consider popup hierarchy (do not
	...
	ImGuiHoveredFlags_AllowWhenBlockedByPopup = 1 << 5, // Return true even if a popup window is normally blocking
	...
	ImGuiHoveredFlags_AllowWhenOverlappedByItem = 1 << 8, // IsItemHovered() only: Return true even if the item uses
	                                                      // AllowOverlap mode and is overlapped by another hoverable
	                                                      // item.
	...
	ImGuiHoveredFlags_NoNavOverride = 1 << 11, // IsItemHovered() only: Disable using gamepad/keyboard navigation state
	...
	ImGuiHoveredFlags_ForTooltip = 1 << 12, // Shortcut for standard flags when using IsItemHovered() + SetTooltip()
	                                        // sequence.
	...
	ImGuiHoveredFlags_DelayNormal = 1 << 16, // IsItemHovered() only: Return true after style.HoverDelayNormal elapsed
	                                         // (~0.40 sec) (shared between items) + requires mouse to be stationary for
	                                         // style.HoverStationaryDelay (once per item).
	ImGuiHoveredFlags_NoSharedDelay = 1 << 17, // IsItemHovered() only: Disable shared delay system where moving from
	                                           // one item to the next keeps the previous timer for a short time
...
enum ImGuiDragDropFlags_
{
	...
	ImGuiDragDropFlags_SourceNoPreviewTooltip = 1 << 0, // Disable preview tooltip. By default, a successful call to
	                                                    // BeginDragDropSource opens a tooltip so you can display a
	                                                    // preview or description of the source contents. This flag
	                                                    // disables this behavior.
	ImGuiDragDropFlags_SourceNoDisableHover = 1 << 1, // By default, when dragging we clear data so that IsItemHovered()
	...
	ImGuiDragDropFlags_SourceNoHoldToOpenOthers = 1 << 2, // Disable the behavior that allows to open tree nodes and
	...
	ImGuiDragDropFlags_PayloadNoCrossContext = 1 << 6, // Hint to specify that the payload may not be copied outside
	                                                   // current dear imgui context.
	ImGuiDragDropFlags_PayloadNoCrossProcess = 1 << 7, // Hint to specify that the payload may not be copied outside
	                                                   // current process.
	...
	ImGuiDragDropFlags_AcceptNoDrawDefaultRect = 1 << 11, // Do not draw the default highlight rectangle when hovering
	                                                      // over target.
	ImGuiDragDropFlags_AcceptNoPreviewTooltip = 1 << 12, // Request hiding the BeginDragDropSource tooltip from the
...
enum ImGuiInputFlags_
{
	...
	ImGuiInputFlags_RouteGlobal = 1 << 12, // Global route (unless a focused window or active item registered the
	                                       // route).
	...
	ImGuiInputFlags_RouteOverActive = 1 << 15, // Option: global route: higher priority than active item. Unlikely you
	...
	ImGuiInputFlags_RouteUnlessBgFocused = 1 << 16, // Option: global route: will not be applied if underlying
	                                                // background/void is focused (== no Dear ImGui windows are
	                                                // focused). Useful for overlay applications.
	ImGuiInputFlags_RouteFromRootWindow = 1 << 17, // Option: route evaluated from the point of view of root window
	                                               // rather than current window.

	// Flags for SetNextItemShortcut()
	ImGuiInputFlags_Tooltip = 1 << 18, // Automatically display a tooltip when hovering item [BETA] Unsure of right api
	                                   // (opt-in/opt-out)
};
...
enum ImGuiConfigFlags_
{
	...
	ImGuiConfigFlags_NavEnableSetMousePos = 1 << 2, // Instruct navigation to move the mouse cursor. May be useful on
	                                                // TV/console systems where moving a virtual mouse is awkward. Will
	                                                // update io.MousePos and set io.WantSetMousePos=true. If enabled
	                                                // you MUST honor io.WantSetMousePos requests in your backend,
	                                                // otherwise ImGui will react as if the mouse is jumping around back
	                                                // and forth.
...
enum ImGuiBackendFlags_
{
	...
	ImGuiBackendFlags_HasMouseCursors = 1 << 1, // Backend Platform supports honoring GetMouseCursor() value to change
	                                            // the OS cursor shape.
	ImGuiBackendFlags_HasSetMousePos = 1 << 2, // Backend Platform supports io.WantSetMousePos requests to reposition
	                                           // the OS mouse position (only used if
	                                           // ImGuiConfigFlags_NavEnableSetMousePos is set).
	ImGuiBackendFlags_RendererHasVtxOffset = 1 << 3, // Backend Renderer supports ImDrawCmd::VtxOffset. This enables
...
enum ImGuiColorEditFlags_
{
	...
	ImGuiColorEditFlags_NoTooltip = 1 << 6, //              // ColorEdit, ColorPicker, ColorButton: disable tooltip when
	                                        //              hovering the preview.
	ImGuiColorEditFlags_NoLabel = 1 << 7, //              // ColorEdit, ColorPicker: disable display of inline text
	                                      //              label (the label is still forwarded to the tooltip and
	                                      //              picker).
	ImGuiColorEditFlags_NoSidePreview = 1 << 8, //              // ColorPicker: disable bigger color preview on right
	                                            //              side of the picker, use small color square preview
	                                            //              instead.
	ImGuiColorEditFlags_NoDragDrop = 1 << 9, //              // ColorEdit: disable drag and drop target. ColorButton:
	                                         //              disable drag and drop source.
	ImGuiColorEditFlags_NoBorder = 1 << 10, //              // ColorButton: disable border (which is enforced by
	                                        //              default)

	// User Options (right-click on widget to change some of them).
	ImGuiColorEditFlags_AlphaBar = 1 << 16, //              // ColorEdit, ColorPicker: show vertical alpha bar/gradient
	                                        //              in picker.
	ImGuiColorEditFlags_AlphaPreview = 1 << 17, //              // ColorEdit, ColorPicker, ColorButton: display preview
	                                            //              as a transparent color over a checkerboard, instead of
	                                            //              opaque.
	...
	ImGuiColorEditFlags_Uint8 = 1 << 23, // [DataType]   // ColorEdit, ColorPicker, ColorButton: _display_ values
	                                     // formatted as 0..255.
	ImGuiColorEditFlags_Float = 1 << 24, // [DataType]   // ColorEdit, ColorPicker, ColorButton: _display_ values
	                                     // formatted as 0.0f..1.0f floats instead of 0..255 integers. No round-trip of
	                                     // value via integers.
	ImGuiColorEditFlags_PickerHueBar = 1 << 25, // [Picker]     // ColorPicker: bar for Hue, rectangle for Sat/Value.
	ImGuiColorEditFlags_PickerHueWheel = 1 << 26, // [Picker]     // ColorPicker: wheel for Hue, triangle for Sat/Value.
	ImGuiColorEditFlags_InputRGB = 1 << 27, // [Input]      // ColorEdit, ColorPicker: input and output data in RGB
	                                        // format.
	ImGuiColorEditFlags_InputHSV = 1 << 28, // [Input]      // ColorEdit, ColorPicker: input and output data in HSV
	                                        // format.
...
enum ImGuiSliderFlags_
{
	...
	ImGuiSliderFlags_NoInput = 1 << 7, // Disable CTRL+Click or Enter key allowing to input text directly into the
	                                   // widget.
...
enum ImGuiCond_
{
	...
	ImGuiCond_FirstUseEver = 1 << 2, // Set the variable if the object/window has no persistently saved data (no entry
	                                 // in .ini file)
	ImGuiCond_Appearing = 1 << 3, // Set the variable if the object/window is appearing after being hidden/inactive (or
	                              // the first time)
};
...
enum ImGuiTableFlags_
{
	...
	ImGuiTableFlags_NoSavedSettings = 1 << 4, // Disable persisting columns order, width and sort settings in the .ini
	                                          // file.
	...
	ImGuiTableFlags_RowBg = 1 << 6, // Set each RowBg color with ImGuiCol_TableRowBg or ImGuiCol_TableRowBgAlt
	...
	ImGuiTableFlags_BordersH = ImGuiTableFlags_BordersInnerH | ImGuiTableFlags_BordersOuterH, // Draw horizontal
	                                                                                          // borders.
	...
	ImGuiTableFlags_NoBordersInBodyUntilResize = 1 << 12, // [ALPHA] Disable vertical borders in columns Body until
	                                                      // hovered for resize (borders will always appear in Headers).
	                                                      // -> May move to style Sizing Policy (read above for
	                                                      // defaults)
	...
	ImGuiTableFlags_NoHostExtendX = 1 << 16, // Make outer width auto-fit to columns, overriding outer_size.x value.
	...
	ImGuiTableFlags_NoKeepColumnsVisible = 1 << 18, // Disable keeping column always minimally visible when ScrollX is
	                                                // off and table gets too small. Not recommended if columns are
	                                                // resizable.
	...
	ImGuiTableFlags_HighlightHoveredColumn = 1 << 28, // Highlight column headers when hovered (may evolve into a fuller
	                                                  // highlight)
...
enum ImGuiTableColumnFlags_
{
	...
	ImGuiTableColumnFlags_Disabled = 1 << 0, // Overriding/master disable flag: hide column, won't show in context menu
	                                         // (unlike calling TableSetColumnEnabled() which manipulates the user
	                                         // accessible state)
	ImGuiTableColumnFlags_DefaultHide = 1 << 1, // Default as a hidden/disabled column.
	ImGuiTableColumnFlags_DefaultSort = 1 << 2, // Default as a sorting column.
	ImGuiTableColumnFlags_WidthStretch = 1 << 3, // Column will stretch. Preferable with horizontal scrolling disabled
	...
	ImGuiTableColumnFlags_NoClip = 1 << 8, // Disable clipping for this column (all NoClip columns will render in a same
	                                       // draw command).
	ImGuiTableColumnFlags_NoSort = 1 << 9, // Disable ability to sort on this field (even if ImGuiTableFlags_Sortable is
	                                       // set on the table).
	ImGuiTableColumnFlags_NoSortAscending = 1 << 10, // Disable ability to sort in the ascending direction.
	ImGuiTableColumnFlags_NoSortDescending = 1 << 11, // Disable ability to sort in the descending direction.
	ImGuiTableColumnFlags_NoHeaderLabel = 1 << 12, // TableHeadersRow() will not submit horizontal label for this
	...
	ImGuiTableColumnFlags_PreferSortDescending = 1 << 15, // Make the initial sort direction Descending when first
	                                                      // sorting on this column.
	ImGuiTableColumnFlags_IndentEnable = 1 << 16, // Use current Indent value when entering cell (default for column 0).
	ImGuiTableColumnFlags_IndentDisable = 1 << 17, // Ignore current Indent value when entering cell (default for
	                                               // columns > 0). Indentation changes _within_ the cell will still be
	                                               // honored.
...
struct ImGuiIO
{
	...
	// Input Functions
	IMGUI_API void AddKeyEvent(ImGuiKey key, bool down); // Queue a new key down/up event. Key should be "translated"
	                                                     // (as in, generally ImGuiKey_A matches the key end-user would
	                                                     // use to emit an 'A' character)
	IMGUI_API void AddKeyAnalogEvent(ImGuiKey key, bool down, float v); // Queue a new key down/up event for analog
	...
	IMGUI_API void AddMouseWheelEvent(float wheel_x, float wheel_y); // Queue a mouse wheel update. wheel_y<0: scroll
	...
	IMGUI_API void AddInputCharacterUTF16(ImWchar16 c); // Queue a new character input from a UTF-16 character, it can
	                                                    // be a surrogate
	...
	IMGUI_API void SetKeyEventNativeData(ImGuiKey key,
	                                     int native_keycode,
	                                     int native_scancode,
	                                     int native_legacy_index = -1); // [Optional] Specify index for legacy <1.87
	                                                                    // IsKeyXXX() functions with native indices +
	                                                                    // specify native keycode, scancode.
	IMGUI_API void SetAppAcceptingEvents(bool accepting_events); // Set master flag for accepting key/mouse/text events
	                                                             // (default to true). Useful if you have native dialog
	                                                             // boxes that are interrupting your application
	                                                             // loop/refresh, and you want to disable events being
	                                                             // queued while your app is frozen.
...
#	endif
	...
	ImGuiKeyChord KeyMods; // Key mods flags (any of ImGuiMod_Ctrl/ImGuiMod_Shift/ImGuiMod_Alt/ImGuiMod_Super flags,
	                       // same as io.KeyCtrl/KeyShift/KeyAlt/KeySuper but merged into flags. Read-only, updated by
	                       // NewFrame()
	ImGuiKeyData KeysData[ImGuiKey_KeysData_SIZE]; // Key state for all known keys. Use IsKeyXXX() functions to access
	                                               // this.
...
struct ImGuiListClipper
{
	...
	IMGUI_API void IncludeItemsByIndex(int item_begin, int item_end); // item_end is exclusive e.g. use (42, 42+1) to
...
enum ImGuiMultiSelectFlags_
{
	...
	ImGuiMultiSelectFlags_SingleSelect = 1 << 0, // Disable selecting more than one item. This is available to allow
	...
	ImGuiMultiSelectFlags_NoRangeSelect = 1 << 2, // Disable Shift+selection mouse/keyboard support (useful for
	                                              // unordered 2D selection). With BoxSelect is also ensure contiguous
	                                              // SetRange requests are not combined into one. This allows not
	...
	ImGuiMultiSelectFlags_NoAutoClearOnReselect = 1 << 5, // Disable clearing selection when clicking/selecting an
	                                                      // already selected item
	ImGuiMultiSelectFlags_BoxSelect1d = 1 << 6, // Enable box-selection with same width and same x pos items (e.g. only
	                                            // full row Selectable()). Box-selection works better with little bit of
	                                            // spacing between items hit-box in order to be able to aim at empty
	                                            // space.
	ImGuiMultiSelectFlags_BoxSelect2d = 1 << 7, // Enable box-selection with varying width or varying x pos items
	                                            // support (e.g. different width labels, or 2D layout/grid). This is
	                                            // slower: alters clipping logic so that e.g. horizontal movements will
	...
	ImGuiMultiSelectFlags_ScopeWindow = 1 << 11, // Scope for _BoxSelect and _ClearOnClickVoid is whole window
	                                             // (Default). Use if BeginMultiSelect() covers a whole window or used a
	                                             // single time in same window.
	...
	ImGuiMultiSelectFlags_SelectOnClick = 1 << 13, // Apply selection on mouse down when clicking on unselected item.
	                                               // (Default)
	ImGuiMultiSelectFlags_SelectOnClickRelease = 1 << 14, // Apply selection on mouse release when clicking an
	...
	ImGuiMultiSelectFlags_NavWrapX = 1 << 16, // [Temporary] Enable navigation wrapping on X axis. Provided as a
	                                          // convenience because we don't have a design for the general Nav API for
	                                          // this yet. When the more general feature be public we may obsolete this
...
struct ImGuiMultiSelectIO
{
	...
	ImVector<ImGuiSelectionRequest> Requests; //  ms:w, app:r     /  ms:w  app:r   // Requests to apply to your
	                                          //  selection data.
...
struct ImGuiSelectionRequest
{
	...
	ImGuiSelectionUserData RangeFirstItem; //                  /  ms:w, app:r   // Parameter for SetRange request (this
	                                       //                  is generally == RangeSrcItem when shift selecting from
	                                       //                  top to bottom).
	ImGuiSelectionUserData RangeLastItem; //                  /  ms:w, app:r   // Parameter for SetRange request (this
	                                      //                  is generally == RangeSrcItem when shift selecting from
...
struct ImGuiSelectionBasicStorage
{
	...
	void* UserData; // = NULL   // User data for use by adapter function        // e.g. selection.UserData =
	                // (void*)my_items;
	ImGuiID (*AdapterIndexToStorageId)(ImGuiSelectionBasicStorage* self, int idx); // e.g.
	...
	// Methods
	ImGuiSelectionBasicStorage();
	IMGUI_API void ApplyRequests(ImGuiMultiSelectIO* ms_io); // Apply selection requests coming from BeginMultiSelect()
	...
	IMGUI_API void SetItemSelected(ImGuiID id, bool selected); // Add/remove an item from selection (generally done by
	                                                           // ApplyRequests() function)
	IMGUI_API bool GetNextSelectedItem(void** opaque_it, ImGuiID* out_id); // Iterate selection with 'void* it = NULL;
	                                                                       // ImGuiId id; while
...
enum ImDrawFlags_
{
	...
	ImDrawFlags_RoundCornersTopRight = 1 << 5, // AddRect(), AddRectFilled(), PathRect(): enable rounding top-right
	...
	ImDrawFlags_RoundCornersBottomLeft = 1 << 6, // AddRect(), AddRectFilled(), PathRect(): enable rounding bottom-left
	                                             // corner only (when rounding > 0.0f, we default to all corners). Was
	                                             // 0x04.
	ImDrawFlags_RoundCornersBottomRight = 1 << 7, // AddRect(), AddRectFilled(), PathRect(): enable rounding
...
enum ImDrawListFlags_
{
	...
	ImDrawListFlags_AntiAliasedLines = 1 << 0, // Enable anti-aliased lines/borders (*2 the number of triangles for 1.0f
	                                           // wide line or lines thin enough to be drawn using textures, otherwise
	...
	ImDrawListFlags_AntiAliasedFill = 1 << 2, // Enable anti-aliased edge around filled shapes (rounded rectangles,
	                                          // circles).
...
struct ImDrawList
{
	...
	ImVector<ImDrawCmd> CmdBuffer; // Draw commands. Typically 1 command = 1 GPU draw call, unless the command is a
	                               // callback.
	...
	ImDrawCmdHeader _CmdHeader; // [Internal] template of active commands. Fields should match those of
	                            // CmdBuffer.back().
	...
	IMGUI_API void
	PathBezierCubicCurveTo(const ImVec2& p2, const ImVec2& p3, const ImVec2& p4, int num_segments = 0); // Cubic Bezier
	                                                                                                    // (4 control
	                                                                                                    // points)
	...
	IMGUI_API void AddCallback(ImDrawCallback callback, void* callback_data); // Your rendering function must check for
	                                                                          // 'UserCallback' in ImDrawCmd and call
	                                                                          // the function instead of rendering
	                                                                          // triangles.
	...
	IMGUI_API void PrimRect(const ImVec2& a, const ImVec2& b, ImU32 col); // Axis aligned rectangle (composed of two
	                                                                      // triangles)
...
struct ImDrawData
{
	...
	IMGUI_API void AddDrawList(ImDrawList* draw_list); // Helper to add an external draw list into an existing
	                                                   // ImDrawData.
	...
	IMGUI_API void ScaleClipRects(const ImVec2& fb_scale); // Helper to scale the ClipRect field of each ImDrawCmd. Use
...
struct ImFontGlyph
{
	...
	unsigned int Visible : 1; // Flag to indicate glyph has no visible pixels (e.g. space). Allow early out when
	                          // rendering.
...
struct ImFontGlyphRangesBuilder
{
	...
	IMGUI_API void AddText(const char* text, const char* text_end = NULL); // Add string (each character of the UTF-8
	                                                                       // string are added)
	IMGUI_API void AddRanges(const ImWchar* ranges); // Add ranges, e.g.
	                                                 // builder.AddRanges(ImFontAtlas::GetGlyphRangesDefault()) to force
...
enum ImFontAtlasFlags_
{
	...
	ImFontAtlasFlags_NoMouseCursors = 1 << 1, // Don't build software mouse cursors into the atlas (save a little
	                                          // texture memory)
...
struct ImFontAtlas
{
	...
	IMGUI_API ImFont* AddFontFromMemoryTTF(void* font_data,
	                                       ...
	                                       const ImWchar* glyph_ranges = NULL); // Note: Transfer ownership of
	                                                                            // 'ttf_data' to ImFontAtlas! Will be
	                                                                            // deleted after destruction of the
	                                                                            // atlas. Set
	                                                                            // font_cfg->FontDataOwnedByAtlas=false
	                                                                            // to keep ownership of your data and it
	                                                                            // won't be freed.
	IMGUI_API ImFont* AddFontFromMemoryCompressedTTF(const void* compressed_font_data,
	                                                 int compressed_font_data_size,
	                                                 float size_pixels,
	                                                 const ImFontConfig* font_cfg = NULL,
	                                                 const ImWchar* glyph_ranges = NULL); // 'compressed_font_data'
	                                                                                      // still owned by caller.
	                                                                                      // Compress with
	...
	unsigned char* TexPixelsAlpha8; // 1 component per pixel, each component is unsigned 8-bit. Total size = TexWidth *
	                                // TexHeight
...
enum ImGuiViewportFlags_
{
	...
	ImGuiViewportFlags_OwnedByApp = 1 << 2, // Platform Window: is created/managed by the application (rather than a
	                                        // dear imgui backend)
};
...
namespace ImGui
{
	...
	IMGUI_API ImVec2 GetWindowContentRegionMin(); // Content boundaries min for the window (roughly (0,0)-Scroll), in
	                                              ...
	                                              // GetCursorScreenPos() and GetContentRegionAvail()!
	IMGUI_API ImVec2 GetWindowContentRegionMax(); // Content boundaries max for the window (roughly (0,0)+Size-Scroll),
	...
	IMGUI_API bool ImageButton(ImTextureID user_texture_id,
	                           ...
	                           int frame_padding = -1,
	                           const ImVec4& bg_col = ImVec4(0, 0, 0, 0),
	                           const ImVec4& tint_col = ImVec4(1, 1, 1, 1)); // Use new ImageButton() signature
	...
	IMGUI_API ImGuiKey GetKeyIndex(ImGuiKey key); // Map ImGuiKey_* values into legacy native key index. ==
	                                              ...
	                                              // calling GetKeyIndex() with ANY ImGuiKey_XXXX values will return the
	                                              // same value!
