...
#		pragma GCC diagnostic ignored "-Wdeprecated-enum-enum-conversion" // warning: bitwise operation between
		                                                                   // different enumeration types ('XXXFlags_'
		                                                                   // and 'XXXFlagsPrivate_') is deprecated
#	endif
...
{
	ImGuiContext& g = *GImGui;
	const bool need_backup = (g.CurrentWindow->DC.TextWrapPos < 0.0f); // Keep existing wrap position if one is already
	                                                                   // set
	if (need_backup)
		...
		if (!(flags & ImGuiButtonFlags_NoKeyModifiers) || (!g.IO.KeyCtrl && !g.IO.KeyShift && !g.IO.KeyAlt))
		{
			if (mouse_button_clicked != -1 && g.ActiveId != id)
			{
				...
				if (flags & (ImGuiButtonFlags_PressedOnClickRelease | ImGuiButtonFlags_PressedOnClickReleaseAnywhere))
				{
					...
					{
						FocusWindow(window, ImGuiFocusRequestFlags_RestoreFocusedChild); // Still need to focus and
						                                                                 // bring to front, but try to
						                                                                 // avoid losing NavId when
						                                                                 // navigating a child
					}
				}
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	...
	if ((flags & ImGuiButtonFlags_AlignTextBaseLine)
	    && style.FramePadding.y < window->DC.CurrLineTextBaseOffset) // Try to vertically align buttons that are
	                                                                 ...
	                                                                 // matches (bit hacky, since it shouldn't be a
	                                                                 // flag)
                        ...
                        ImDrawFlags flags)
{
	...
	float grab_v_norm = scroll_ratio * (scrollbar_size_v - grab_h_pixels) / scrollbar_size_v; // Grab position in
	                                                                                          // normalized space
...
bool ImGui::TextLink(const char* label)
{
	...
	window->DrawList->AddLine(ImVec2(bb.Min.x, line_y), ImVec2(bb.Max.x, line_y), GetColorU32(line_colf)); // FIXME-TEXT:
	                                                                                                       // Underline
	                                                                                                       // mode.
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags, float thickness)
{
	...
	ImGuiContext& g = *GImGui;
	IM_ASSERT(ImIsPowerOfTwo(flags & (ImGuiSeparatorFlags_Horizontal | ImGuiSeparatorFlags_Vertical))); // Check that
	                                                                                                    // only 1 option
	                                                                                                    // is selected
	...
	else if (flags & ImGuiSeparatorFlags_Horizontal)
	{
		...
		const float thickness_for_layout = (thickness == 1.0f) ? 0.0f : thickness; // FIXME: See 1.70/1.71 Separator()
		                                                                           // change: makes legacy 1-px
		                                                                           // separator not affect layout yet.
		                                                                           // Should change.
...
void ImGui::SeparatorTextEx(ImGuiID id, const char* label, const char* label_end, float extra_w)
{
	...
	const float text_baseline_y = ImTrunc((bb.GetHeight() - label_size.y) * style.SeparatorTextAlign.y
	                                      + 0.99999f); // ImMax(padding.y,
	                                                   // ImFloor((style.SeparatorTextSize
	                                                   // - label_size.y) * 0.5f));
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
	...
	if (ImGuiWindow* popup_window = FindWindowByName(name))
		if (popup_window->WasActive)
		{
			...
			popup_window->AutoPosLastDirection =
			    (flags & ImGuiComboFlags_PopupAlignLeft) ? ImGuiDir_Left : ImGuiDir_Down; // Left = "Below, Toward
			                                                                              // Left", Down = "Below,
	...
	PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(g.Style.FramePadding.x, g.Style.WindowPadding.y)); // Horizontally
	                                                                                                    // align
	                                                                                                    // ourselves
	                                                                                                    // with the
	                                                                                                    // framed text
...
bool ImGui::BeginComboPreview()
{
	...
	IM_ASSERT(g.LastItemData.Rect.Min.x == preview_data->PreviewRect.Min.x
	          && g.LastItemData.Rect.Min.y == preview_data->PreviewRect.Min.y); // Didn't call after BeginCombo/EndCombo
	                                                                            // block or forgot to pass
                                  ...
                                  float zero_deadzone_halfsize)
{
	...
	if (is_logarithmic)
	{
		...
		{
			float zero_point_center = (-(float)v_min) / ((float)v_max - (float)v_min); // The zero point in parametric
			                                                                           // space.  There's an argument we
			                                                                           // should take the logarithmic
			                                                                           // nature into account when
			                                                                           // calculating this, but for now
			                                                                           // this should do (and the most
			                                                                           // common case of a symmetrical
			                                                                           // range works fine)
                                 ...
                                 float zero_deadzone_halfsize)
{
	...
	if (is_logarithmic)
	{
		...
		float t_with_flip = flipped ? (1.0f - t) : t; // t, but flipped if necessary to account for us flipping the
		                                              // range

		if ((v_min * v_max) < 0.0f) // Range crosses zero, so we have to do this in two parts
		{
			float zero_point_center = (-(float)ImMin(v_min, v_max)) / ImAbs((float)v_max - (float)v_min); // The zero
			                                                                                              // point in
			                                                                                              // parametric
			                                                                                              // space
			float zero_point_snap_L = zero_point_center - zero_deadzone_halfsize;
                            ...
                            ImRect* out_grab_bb)
{
	...
	const bool is_logarithmic = (flags & ImGuiSliderFlags_Logarithmic) != 0;
	const bool is_floating_point = (data_type == ImGuiDataType_Float) || (data_type == ImGuiDataType_Double);
	const float v_range_f = (float)(v_min < v_max ? v_max - v_min : v_min - v_max); // We don't need high precision for
	                                                                                // what we do with it.
	...
	if (!is_floating_point && v_range_f >= 0.0f) // v_range_f < 0 may happen on integer overflows
		grab_sz = ImMax(slider_sz / (v_range_f + 1), style.GrabMinSize); // For integer sliders: if possible have the
	...
	if (g.ActiveId == id)
	{
			...
			else
			{
				...
				if (g.ActiveIdIsJustActivated)
				{
					...
					const bool clicked_around_grab = (mouse_abs_pos >= grab_pos - grab_sz * 0.5f - 1.0f)
					                                 && (mouse_abs_pos <= grab_pos + grab_sz * 0.5f + 1.0f); // No harm
					                                                                                         // being
					                                                                                         // extra
					                                                                                         // generous
					                                                                                         // here.
		...
		else if (g.ActiveIdSource == ImGuiInputSource_Keyboard || g.ActiveIdSource == ImGuiInputSource_Gamepad)
		{
			...
			if (input_delta != 0.0f)
			{
				...
				else
				{
					if ((v_range_f >= -100.0f && v_range_f <= 100.0f && v_range_f != 0.0f) || tweak_slow)
						input_delta = ((input_delta < 0.0f) ? -1.0f : +1.0f) / v_range_f; // Gamepad/keyboard tweak
			...
			else if (g.SliderCurrentAccumDirty)
			{
				...
				if ((clicked_t >= 1.0f && delta > 0.0f) || (clicked_t <= 0.0f && delta < 0.0f)) // This is to avoid
				                                                                                // applying the
				                                                                                // saturation when
				                                                                                // already past the
				                                                                                // limits
                        ...
                        ImGuiInputTextFlags flags)
{
	...
	void* p_data_default = (g.NextItemData.Flags & ImGuiNextItemDataFlags_HasRefVal) ? &g.NextItemData.RefVal
	                                                                                 : &g.DataTypeZeroValue;
	...
	if (p_step == NULL)
	{
		if (InputText(label, buf, IM_ARRAYSIZE(buf), flags))
			value_changed = DataTypeApplyFromText(buf,
			                                      data_type,
			                                      p_data,
			                                      format,
	...
	else
	{
		...
		if (InputText("", buf, IM_ARRAYSIZE(buf), flags)) // PushId(label) + "" gives us the expected ID from outside
		                                                  // point of view
			value_changed = DataTypeApplyFromText(buf,
			                                      data_type,
			                                      p_data,
                                     ...
                                     bool stop_on_new_line)
{
	...
	if (out_offset)
		*out_offset = ImVec2(line_width, text_size.y + line_height); // offset allow for the possibility of sitting
		                                                             // after a trailing \n
                                     ...
                                     bool input_source_is_clipboard)
{
	...
	{
		bool pass = false;
		pass |= (c == '\n') && (flags & ImGuiInputTextFlags_Multiline) != 0; // Note that an Enter KEY will emit \r and
		                                                                     // be ignored (we poll for KEY in
		                                                                     // InputText() code)
                        ...
                        void* callback_user_data)
{
	...
	IM_ASSERT(!((flags & ImGuiInputTextFlags_CallbackHistory) && (flags & ImGuiInputTextFlags_Multiline))); // Can't use
	                                                                                                        // both
	                                                                                                        // together
	                                                                                                        // (they
	                                                                                                        // both use
	...
	const ImVec2 frame_size =
	    CalcItemSize(size_arg,
	                 CalcItemWidth(),
	                 (is_multiline ? g.FontSize * 8.0f : label_size.y) + style.FramePadding.y * 2.0f); // Arbitrary
	                                                                                                   // default of 8
	                                                                                                   // lines high for
	                                                                                                   // multi-line
	const ImVec2 total_size =
	...
	const bool init_changed_specs = (state != NULL && state->Stb.single_line != !is_multiline); // state != NULL means
	                                                                                            // its our state.
	...
	if ((init_state && g.ActiveId != id) || init_changed_specs || init_reload_from_user_buf)
	{
		...
		if (!init_reload_from_user_buf)
		{
			...
			state->InitialTextA.resize(buf_len + 1); // UTF-8. we use +1 to make sure that .Data is always pointing to
	...
	if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
	{
		...
		const bool is_wordmove_key_down = is_osx ? io.KeyAlt : io.KeyCtrl; // OS X style: Text editing cursor movement
		                                                                   // using Alt instead of Ctrl
		const bool is_startend_key_down = is_osx && io.KeyCtrl && !io.KeySuper && !io.KeyAlt; // OS X style: Line/Text
		                                                                                      // Start and End using
		                                                                                      // Cmd+Arrows instead of
		                                                                                      // Home/End
					...
					if (buf_dirty)
					{
						...
						if (callback_data.BufTextLen > backup_current_text_length && is_resizable)
							state->TextW.resize(state->TextW.Size
							                    + (callback_data.BufTextLen - backup_current_text_length)); // Worse
							                                                                                // case
							                                                                                // scenario
							                                                                                // resize
	...
	if (render_cursor || render_selection)
	{
		...
		if (render_cursor && state->CursorFollow)
		{
			...
			if (is_multiline)
			{
				...
				draw_pos.y += (draw_window->Scroll.y - scroll_y); // Manipulate cursor pos immediately avoid a frame of
				                                                  // lag
		...
		if (render_selection)
		{
			...
			ImU32 bg_color = GetColorU32(ImGuiCol_TextSelectedBg, render_cursor ? 1.0f : 0.6f); // FIXME: current code
			                                                                                    // flow mandate that
			                                                                                    // render_cursor is
			                                                                                    // always true here, we
			                                                                                    // are leaving the
			...
			for (const ImWchar* p = text_selected_begin; p < text_selected_end;)
			{
				...
				else
				{
					...
					if (rect_size.x <= 0.0f)
						rect_size.x = IM_TRUNC(g.Font->GetCharAdvance((ImWchar)' ') * 0.50f); // So we can see selected
						                                                                      // empty lines
	...
	else
	{
		...
		if (is_multiline)
			text_size = ImVec2(inner_size.x,
			                   InputTextCalcTextLenAndLineCount(buf_display, &buf_display_end) * g.FontSize); // We
			                                                                                                  // don't
			                                                                                                  // need
			                                                                                                  // width
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
	...
	if (value_changed && g.LastItemData.ID != 0) // In case of ID collision, the second EndGroup() won't catch
	                                             // g.ActiveId
		MarkItemEdited(g.LastItemData.ID);
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
	...
	float sv_picker_size =
	    ImMax(bars_width * 1, width - (alpha_bar ? 2 : 1) * (bars_width + style.ItemInnerSpacing.x)); // Saturation/Value
	                                                                                                  // picking box
	...
	ImU32 user_col32_striped_of_alpha = ColorConvertFloat4ToU32(ImVec4(R, G, B, style.Alpha)); // Important: this is
	                                                                                           // still including the
	...
	if (flags & ImGuiColorEditFlags_PickerHueWheel)
	{
		...
		int hue_cursor_segments = draw_list->_CalcCircleAutoSegmentCount(hue_cursor_rad); // Lock segment count so the
		                                                                                  // +1 one matches others.
	...
	else if (flags & ImGuiColorEditFlags_PickerHueBar)
	{
		...
		sv_cursor_pos.x = ImClamp(IM_ROUND(picker_pos.x + ImSaturate(S) * sv_picker_size),
		                          picker_pos.x + 2,
		                          picker_pos.x + sv_picker_size - 2); // Sneakily prevent the circle to stick out too
		                                                              // much
	...
	int sv_cursor_segments = draw_list->_CalcCircleAutoSegmentCount(sv_cursor_rad); // Lock segment count so the +1 one
	                                                                                // matches others.
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
	...
	if ((flags & ImGuiColorEditFlags_NoBorder) == 0)
	{
		if (g.Style.FrameBorderSize > 0.0f)
			...
			window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(ImGuiCol_FrameBg), rounding); // Color button are
			                                                                                    // often in need of some
			                                                                                    // sort of border
	}
...
void ImGui::ColorPickerOptionsPopup(const float* ref_col, ImGuiColorEditFlags flags)
{
	...
	if (allow_opt_picker)
	{
		ImVec2 picker_size(g.FontSize * 8,
		                   ImMax(g.FontSize * 8 - (GetFrameHeight() + g.Style.ItemInnerSpacing.x), 1.0f)); // FIXME:
		                                                                                                   // Picker
		                                                                                                   // size
		                                                                                                   // copied
		                                                                                                   // from main
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiID storage_id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
	...
	const float text_offset_x = g.FontSize + (display_frame ? padding.x * 3 : padding.x * 2); // Collapsing arrow width
	                                                                                          // + Spacing
	...
	if (display_frame)
	{
		const float outer_extend = IM_TRUNC(window->WindowPadding.x * 0.5f); // Framed header expand a little outside of
		                                                                     // current limits
	...
	if ((flags
	     & (ImGuiTreeNodeFlags_Framed | ImGuiTreeNodeFlags_SpanAvailWidth | ImGuiTreeNodeFlags_SpanFullWidth
	        | ImGuiTreeNodeFlags_SpanTextWidth | ImGuiTreeNodeFlags_SpanAllColumns))
	    == 0)
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	...
	if (!is_visible)
		if (!is_multi_select || !g.BoxSelectState.UnclipMode
		    || !g.BoxSelectState.UnclipRect.Overlaps(bb)) // Extra layer of "no logic clip" for box-select support
	...
	if (pressed || (hovered && (flags & ImGuiSelectableFlags_SetNavIdOnHover)))
	{
		...
		{
			SetNavID(id, window->DC.NavLayerCurrent, g.CurrentFocusScopeId, WindowRectAbsToRel(window, bb)); // (bb ==
			                                                                                                 // NavRect)
			g.NavDisableHighlight = true;
		}
	}
...
ImGuiTypingSelectRequest* ImGui::GetTypingSelectRequest(ImGuiTypingSelectFlags flags)
{
	...
	const int TYPING_SELECT_SINGLE_CHAR_COUNT_FOR_LOCK = 4; // Lock single char matching when repeating same char 4
	                                                        // times
	...
	for (ImWchar w : g.IO.InputQueueCharacters)
	{
		...
		if (w < 32 || (buffer_len == 0 && ImCharIsBlankW(w)) || (buffer_len + w_len > buffer_max_len)) // Ignore leading
		                                                                                               // blanks
			continue;
		char w_buf[5];
	...
	if (flags & ImGuiTypingSelectFlags_AllowSingleCharMode)
	{
		...
		data->SingleCharModeLock |= (single_char_count >= TYPING_SELECT_SINGLE_CHAR_COUNT_FOR_LOCK); // From now on we
		                                                                                             // stop search
		                                                                                             // matching to lock
		                                                                                             // to single char
		                                                                                             // mode.
                                               ...
                                               int nav_item_idx)
{
	...
	for (int idx = 0; idx < items_count; idx++)
	{
		...
		if (return_next_match) // Return next matching item after current item.
			return idx;
		if (first_match_idx == -1 && nav_item_idx == -1) // Return first match immediately if we don't have a
		                                                 // nav_item_idx value.
			return idx;
...
static void BoxSelectScrollWithMouseDrag(ImGuiBoxSelectState* bs, ImGuiWindow* window, const ImRect& inner_r)
{
	...
	for (int n = 0; n < 2; n++) // each axis
	{
		...
		const float speed_multiplier =
		    ImLinearRemapClamp(g.FontSize, g.FontSize * 5.0f, 1.0f, 4.0f, ImAbs(dist)); // x1 to x4 depending on
		                                                                                // distance
...
void ImGui::EndBoxSelect(const ImRect& scope_rect, ImGuiMultiSelectFlags ms_flags)
{
	...
	bs->EndPosRel = WindowPosAbsToRel(window, ImClamp(g.IO.MousePos, scope_rect.Min, scope_rect.Max)); // Clamp stored
	                                                                                                   // position
	                                                                                                   // according to
	                                                                                                   // current
	                                                                                                   // scrolling view
	...
	window->DrawList->AddRect(box_select_r.Min, box_select_r.Max, GetColorU32(ImGuiCol_NavHighlight)); // FIXME-MULTISELECT:
	                                                                                                   // Styling
...
ImGuiMultiSelectIO* ImGui::EndMultiSelect()
{
	...
	const ImRect scope_rect = (ms->Flags & ImGuiMultiSelectFlags_ScopeRect)
	                              ? ImRect(ms->ScopeRectMin, ImMax(window->DC.CursorMaxPos, ms->ScopeRectMin))
	...
	if (ms->IsFocused)
	{
		...
		if (ms->IO.RangeSrcReset
		    || (ms->RangeSrcPassedBy == false
		        && ms->IO.RangeSrcItem != ImGuiSelectionUserData_Invalid)) // Can't read
		...
		if (ms->Flags & (ImGuiMultiSelectFlags_BoxSelect1d | ImGuiMultiSelectFlags_BoxSelect2d))
		{
			if (!g.BoxSelectState.IsActive && !g.BoxSelectState.IsStarting && g.IO.MouseClickedCount[0] == 1)
			{
				...
				if (ms->Flags & ImGuiMultiSelectFlags_ScopeRect)
					SetNavID(0, ImGuiNavLayer_Main, ms->FocusScopeId, ImRect(g.IO.MousePos, g.IO.MousePos)); // Automatically
					                                                                                         // switch
					                                                                                         // FocusScope
					                                                                                         // for
					                                                                                         // initial
...
void ImGui::MultiSelectItemHeader(ImGuiID id, bool* p_selected, ImGuiButtonFlags* p_button_flags)
{
	...
	if (ms->IsFocused)
	{
		...
		if (ms->IsKeyboardSetRange)
		{
			...
			const bool is_range_dst = (ms->RangeDstPassedBy == false) && g.NavJustMovedToId == id; // Assume that
			                                                                                       // g.NavJustMovedToId
			                                                                                       // is not clipped.
			if (is_range_dst)
				ms->RangeDstPassedBy = true;
			if (is_range_dst && storage->RangeSrcItem == ImGuiSelectionUserData_Invalid) // If we don't have RangeSrc,
...
void ImGui::MultiSelectItemFooter(ImGuiID id, bool* p_selected, bool* p_pressed)
{
	...
	{
		// Box-select
		ImGuiInputSource input_source = (g.NavJustMovedToId == id || g.NavActivateId == id) ? g.NavInputSource
		...
		if ((flags & ImGuiMultiSelectFlags_NoAutoClear) == 0)
		{
			...
			if (is_singleselect)
				...
				request_clear = (flags & ImGuiMultiSelectFlags_NoAutoClearOnReselect) ? !selected : true;
			         ...
			         && !is_ctrl)
				request_clear = true; // With is_shift==false the RequestClear was done in BeginIO, not necessary to do
				                      // again.
			if (request_clear)
				MultiSelectAddSetAll(ms, false);
		}
...
{
#	ifndef IMGUI_DISABLE_DEBUG_TOOLS
	const bool is_active = (storage->LastFrameActive >= GetFrameCount() - 2); // Note that fully clipped early out
	                                                                          // scrolling tables will appear as
	                                                                          // inactive here.
	if (!is_active)
	{
                                                            ...
                                                            int selection_order)
{
	...
	if (selected && !is_contained)
		storage->Data.push_back(ImGuiStoragePair(id, selection_order)); // Push unsorted at end of vector, will be
...
static void
ImGuiSelectionBasicStorage_BatchFinish(ImGuiSelectionBasicStorage* selection, bool selected, int size_before_amends)
{
	ImGuiStorage* storage = &selection->_Storage;
                    ...
                    int height_in_items)
{
	...
	clipper.Begin(items_count, GetTextLineHeightWithSpacing()); // We know exactly our line height here so we pass it as
	                                                            // a minor optimization, but generally you don't need
	                                                            // to.
	clipper.IncludeItemByIndex(*current_item);
	while (clipper.Step())
		for (int i = clipper.DisplayStart; i < clipper.DisplayEnd; i++)
                  ...
                  const ImVec2& size_arg)
{
	...
	if (values_count >= values_count_min)
	{
		...
		float histogram_zero_line_t = (scale_min * scale_max < 0.0f) ? (1 + scale_min * inv_scale)
		                                                             : (scale_min < 0.0f ? 0.0f : 1.0f); // Where does
		                                                                                                 // the zero
		                                                                                                 // line stands
...
void ImGui::EndMenuBar()
{
	    ...
	    && (g.NavWindow->Flags & ImGuiWindowFlags_ChildMenu))
	{
		    ...
		    && (g.NavMoveFlags & ImGuiNavMoveFlags_Forwarded) == 0)
		{
			...
			g.NavDisableHighlight = true; // Hide highlight for the current frame so we don't see the intermediary
			                              // selection.
	...
	PopClipRect();
	PopID();
	window->DC.MenuBarOffset.x = window->DC.CursorPos.x - window->Pos.x; // Save horizontal position so next append can
	...
	window->DC.IdealMaxPos.x =
	    ImMax(window->DC.IdealMaxPos.x, window->DC.CursorMaxPos.x - window->Scroll.x); // Convert ideal extents for
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
	...
	if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
	{
		...
		PopStyleVar();
		window->DC.CursorPos.x += IM_TRUNC(style.ItemSpacing.x * (-1.0f + 0.5f)); // -1 spacing to compensate the
	...
	else
	{
		...
		float min_w = window->DC.MenuColumns.DeclColumns(icon_w, label_size.x, 0.0f, checkmark_w); // Feedback to next
		                                                                                           // frame
	                                                       ...
	                                                       // (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
	{
		...
		if (g.HoveredWindow == window && child_menu_window != NULL)
		{
			...
			tb.y = ta.y + ImMax((tb.y - pad_farmost_h) - ta.y, -ref_unit * 8.0f); // Triangle has maximum height to
			                                                                      // limit the slope and the bias toward
			                                                                      // large sub-menus
		...
		if (!menu_is_open && pressed) // Click/activate to open
			...
			want_open = true;
		else if (!menu_is_open && hovered && g.HoveredIdTimer >= 0.30f && g.MouseStationaryTimer >= 0.30f) // Hover to
		                                                                                                   // open
		                                                                                                   // (timer
		                                                                                                   // fallback)
			want_open = true;
	...
	{
		menu_is_open = true;
		OpenPopup(label, ImGuiPopupFlags_NoReopen); // | (want_open_nav_init ? ImGuiPopupFlags_NoReopenAlwaysNavInit :
		                                            // 0));
	}
	...
	if (menu_is_open)
	{
		...
		PushStyleVar(ImGuiStyleVar_ChildRounding, style.PopupRounding); // First level will use _PopupRounding,
...
bool ImGui::MenuItemEx(const char* label, const char* icon, const char* shortcut, bool selected, bool enabled)
{
	...
	else
	{
		...
		float checkmark_w = IM_TRUNC(g.FontSize * 1.20f);
		float min_w = window->DC.MenuColumns.DeclColumns(icon_w, label_size.x, shortcut_w, checkmark_w); // Feedback for
		                                                                                                 // next frame
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
	...
	if (central_section_is_visible)
		width_excess = ImMax(section_1_w - (tab_bar->BarRect.GetWidth() - section_0_w - section_2_w), 0.0f); // Excess
		                                                                                                     // used to
		                                                                                                     // shrink
		                                                                                                     // central
		                                                                                                     // section
	else
		width_excess = (section_0_w + section_2_w) - tab_bar->BarRect.GetWidth(); // Excess used to shrink
...
bool ImGui::BeginTabItem(const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
	...
	IM_ASSERT(!(flags & ImGuiTabItemFlags_Button)); // BeginTabItem() Can't be used with button flags, use
...
bool ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
	...
	if (!is_tab_button)
	{
		...
		if ((flags & ImGuiTabItemFlags_SetSelected) && (tab_bar->SelectedTabId != id)) // _SetSelected can only be
		                                                                               // passed on explicit tab bar
			TabBarQueueFocus(tab_bar, tab);
	}
...
#	endif
	...
	IM_ASSERT(!is_tab_button || !(tab_bar->SelectedTabId == tab->ID && is_tab_button)); // TabItemButton should not be
	                                                                                    // selected
	if (is_tab_button)
		return pressed;
	return tab_contents_visible;
...
ImVec2 ImGui::TabItemCalcSize(const char* label, bool has_close_button_or_unsaved_marker)
{
	...
	if (has_close_button_or_unsaved_marker)
		size.x += g.Style.FramePadding.x + (g.Style.ItemInnerSpacing.x + g.FontSize); // We use Y intentionally to fit
		                                                                              // the close button circle.
	else
